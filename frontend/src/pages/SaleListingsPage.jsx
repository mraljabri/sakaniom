...
export default function SaleListingsPage() {
  ...
  const { t, TYPES, OWNERSHIP_TYPES, SELLER_TYPES, FURNISHED_OPTIONS } = useLanguage();
  ...
  const clearFilters = () => setFilters({ governorate: '', city: '', property_type: '', bedrooms: '', bathrooms: '', min_price: '', max_price: '', furnished: '', ownership_type: '', seller_type: '', search: '', sort: 'newest' });
        ...
        <div className="flex flex-wrap gap-2">
          {TYPES.map(tp => (
            <button key={tp.value} onClick={() => setFilter('property_type', filters.property_type === tp.value ? '' : tp.value)}
              className="chip chip-emerald" aria-pressed={filters.property_type === tp.value}>
              {tp.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {bedOptions.map(b => (
            <button key={b.value} onClick={() => setFilter('bedrooms', filters.bedrooms === b.value ? '' : b.value)}
              className="chip chip-emerald" aria-pressed={filters.bedrooms === b.value}>
              {b.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {['1','2','3','4'].map(b => (
            <button key={b} onClick={() => setFilter('bathrooms', filters.bathrooms === b ? '' : b)}
              className="chip chip-emerald" aria-pressed={filters.bathrooms === b}>
              {b}+
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {SELLER_TYPES.filter(s => s.value).map(s => (
            <button key={s.value} onClick={() => setFilter('seller_type', filters.seller_type === s.value ? '' : s.value)}
              className="chip chip-emerald" aria-pressed={filters.seller_type === s.value}>
              {s.label}
            </button>
          ))}
        </div>
          ...
          <div className="lg:hidden">
            ...
            <div className="sheet" role="dialog" aria-label={t('filter_title')}>
              ...
              <div className="px-5 py-3 border-t border-gray-100">
                <button onClick={() => setSidebarOpen(false)} className="btn-primary w-full !bg-emerald-600 hover:!bg-emerald-700">
