...
export default function CreateSaleListingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append('listing_purpose', 'sale');
      Object.entries(form).forEach(([k, v]) => {
        if (k === 'show_email') data.append(k, v);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('create_sale_title')}</h1>
        <p className="text-gray-500 text-sm mt-1">{t('create_sale_sub')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Section title={t('section_specs')}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="col-span-2">
              ...
              <input className="input" type="number" placeholder="e.g. 85000" value={form.price} onChange={set('price')} required min="1" />
            </div>
            <div>
        ...
        {/* Ownership */}
        <Section title={t('section_ownership')}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="label">{t('sale_ownership_type')} *</label>
              <select className="input" value={form.ownership_type} onChange={set('ownership_type')} required>
                {OWNERSHIP_TYPES.map(o => <option key={o.value} value={o.value} disabled={o.value === ''}>{o.label}</option>)}
              </select>
            </div>
            <div>
              <label className="label">{t('sale_seller_type')} *</label>
              <select className="input" value={form.seller_type} onChange={set('seller_type')} required>
                {SELLER_TYPES.map(s => <option key={s.value} value={s.value} disabled={s.value === ''}>{s.label}</option>)}
              </select>
            </div>
          </div>
        </Section>
        ...
        <Section title={t('section_photos')}>
          <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-emerald-400 transition-colors cursor-pointer"
            onClick={() => document.getElementById('sale-photo-input').click()}>
            <input id="sale-photo-input" type="file" accept="image/*" multiple className="hidden" onChange={handlePhotos} />
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 mt-4">
              {photoPreviews.map((src, i) => (
                <div key={i} className="relative group aspect-square">
                  ...
                  {i === 0 && <span className="absolute bottom-1 start-1 bg-emerald-600 text-white text-xs px-1.5 py-0.5 rounded">{t('cover_label')}</span>}
                </div>
              ))}
            </div>
          )}
        ...
        <Section title={t('section_videos')}>
          <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-emerald-400 transition-colors cursor-pointer"
            onClick={() => document.getElementById('sale-video-input').click()}>
            <input id="sale-video-input" type="file" accept="video/*" multiple className="hidden" onChange={handleVideos} />
            <svg className="w-10 h-10 text-gray-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <Section title={t('section_contact')}>
          <div className="space-y-4">
            ...
            {form.contact_email && (
              <div
                className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors select-none ${form.show_email ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}
                onClick={() => setForm(f => ({ ...f, show_email: !f.show_email }))}
              >
                <div className={`relative w-10 h-6 rounded-full flex-shrink-0 transition-colors ${form.show_email ? 'bg-emerald-600' : 'bg-gray-300'}`}>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={loading} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2.5 px-4 rounded-xl transition-colors disabled:opacity-60">
            {loading ? (
